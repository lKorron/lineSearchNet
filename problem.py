...
def generate_coefficients(n):
    ...
    return [c, generate_a(n), generate_a(n)]


def create_polynomial(c, a1, a2):
    def vector(x):
        return np.array([x, x ** 2, x ** 3, x ** 4, x ** 5])

    return lambda x: (c + np.matmul(np.array(a1), vector(x[0])) +
                      np.matmul(np.array(a2), vector(x[1])))


def create_gradient(c, a1, a2):
    def d_vector(x):
        return np.array([1, 2 * x, 3 * x ** 2, 4 * x ** 3, 5 * x ** 4])

    return lambda x: [np.matmul(np.array(a1), d_vector(x[0])),
                      np.matmul(np.array(a2), d_vector(x[1]))]
...
def load_data():
    ...
    for i in range(n):
        ...
        x = random.randint(-10, 10) / 10 + random.randint(-10, 10)
        y = random.randint(-10, 10) / 10 + random.randint(-10, 10)
...
# pol = create_polynomial(0, [1, 2, 0, 0, 3], [1, 2, 0, 0, 0])
# grad = create_gradient(0, [1, 2, 0, 0, 3], [1, 2, 0, 0, 0])
#
# for i in range(100):
#
#
#     point = np.array([0, 0])
#
#     search = np.array(grad(point)) * -1
#
#     y_real = line_search(pol, grad, point, search)
#
# # print(y_predicted)
#     print(y_real)
